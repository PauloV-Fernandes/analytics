# 📊 Data Analytics & Engineering Portfolio

[PT] Repositório central focado em soluções de dados end-to-end, abrangendo desde a engenharia e modelagem até a visualização estratégica para tomada de decisão.
[EN] Central repository focused on end-to-end data solutions, ranging from engineering and modeling to strategic visualization for decision-making.

---

## 📂 Estrutura de Projetos | Project Structure

### 💰 [Operacional & Financeiro / Operations & Finance](./01-operations-finance)
**PT:** Business Intelligence para gestão de fluxo de caixa e análise de liquidez em tempo real.  
**EN:** Business Intelligence for cash flow management and real-time liquidity analysis.

### 📈 [Comercial & Vendas / Commercial & Sales](./02-commercial-sales)
**PT:** Análise de performance comercial (Northwind) e automação de processos de vendas.  
**EN:** Commercial performance analysis (Northwind) and sales process automation.

### ⚡ [Energia / Energy Sector](./03-energy)
**PT:** Análise técnica e operacional voltada para o setor elétrico e energia renovável.  
**EN:** Technical and operational analysis focused on the electricity and renewable energy sector.

### 🧪 [Ciência de Dados / Data Science Legacy](./04-data-science)
**PT:** Projetos de fundamentos estatísticos, modelagem preditiva e ciência de dados.  
**EN:** Statistical fundamentals, predictive modeling, and data science projects.

---

## 🛠️ Stack Tecnológica | Tech Stack
**Languages:** SQL (PostgreSQL/T-SQL), Python, DAX, Power Query (M)  
**Tools:** Power BI, AWS, Databricks, Git/GitHub  
**Frameworks:** Star Schema Modeling, ETL/ELT Pipelines

---
📫 **Contacto | Contact:** [Seu LinkedIn Aqui]
